...
/**
 * Demo: Adding and deleting rows in a TableView.
 *
 * Refactor summary:
 * - Fixed window title typo ("TableViews" -> "TableView").
 * - Simplified selected indices extraction in deleteSelectedRows by using
 *   the ObservableList's toArray without a redundant preallocated array.
 * - Minor comment cleanups and kept multi-select deletion logic (delete from
 *   highest index down) to avoid index shifting issues.
 *
 * Behavior is unchanged: users can add a new Person from the inputs, delete
 * selected rows (multi-select supported), and restore the original data set.
 */
public class TableViewAddDeleteRows extends Application {
    ...
    public void start(Stage stage) {
        ...
        TableView.TableViewSelectionModel<Person> tsm = table.getSelectionModel();
