...
/**
 * Demo: Editing cells in a TableView with built-in editable cell factories.
 *
 * Refactor summary:
 * - Added missing explicit import for LocalDateStringConverter to clarify intent and avoid reliance on wildcard imports.
 * - Simplified lambdas: in the "Baby?" column, use a one-liner ReadOnlyBooleanWrapper without temporary variables.
 * - Fixed wording in comments and grouped related setup steps for readability (no behavior change).
 * - Kept TextFieldTableCell with LocalDateStringConverter for safe LocalDate <-> String editing.
 *
 * Behavior is unchanged: the table remains editable for name and birth date, shows a non-editable checkbox for
 * "Baby?", and a ComboBox for gender selection with console logging on commit.
 */
public class TableViewEditing extends Application {
    ...
    public void addBirthDateColumn(TableView<Person> table) {
        ...
        // Use a TextFieldTableCell with a LocalDateStringConverter for editing
    ...
    public void addBabyColumn(TableView<Person> table) {
        ...
        // Set a cell value factory (read-only; derived from age category)
    ...
    public void addGenderColumn(TableView<Person> table) {
        ...
        // By default, all cells have null values
        genderCol.setCellValueFactory(cellData -> new ReadOnlyStringWrapper(null));

        // Set a ComboBoxTableCell, so we can select a value from a list
